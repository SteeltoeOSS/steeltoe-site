...
Now for the Config Server! Clone the https://github.com/spring-cloud/spring-cloud-config[spring-cloud-config] repository on GitHub and `cd` into its `spring-cloud-config-server` directory. Then start up the Config Server with the `mvn spring-boot:run` command:
...
https://docs.asp.net/en/latest/client-side/yeoman.html[Generate] a new ASP.NET Core basic web application using Yeoman. For our example purposes, call the application &#8220;Foo&#8221;. Create a `nuget.config` file, and within it, list the Steel Toe feeds:
